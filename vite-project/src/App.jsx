...
const Quiz = () => {
  ...
  const [questionNumber, setQuestionNumber] = useState(1); // Top bar counter
  ...
  const handleNextQuestion = () => {
    ...
    setQuestionNumber((prevNumber) => prevNumber + 1); // Increment the top bar counter
  };

  const renderStars = (difficulty) => {
    const starCount = ['easy', 'medium', 'hard'].indexOf(difficulty) + 1;
    return Array(starCount).fill().map((_, index) => <span key={index}>⭐</span>);
  };
  ...
  if (currentQuestion >= questions.length) {
    return (
      <div className="quiz-container">
        <h2>Quiz Result</h2>
        <p className="result-text">Your score: {score} out of {questions.length}</p>
      </div>
    );
  }
  ...
  return (
    <div className="quiz-container">
      ...
      <div className="question-container">
        <h3>{decodeURIComponent(questions[currentQuestion].question)}</h3>
        ...
        {selectedAnswer !== null && (
          <div className="feedback">
            {selectedAnswer === questions[currentQuestion].correct_answer ? (
              ...
              <p className="incorrect-text">Sorry. Please try again.</p>
            )}
